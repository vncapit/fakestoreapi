<template>
  <div
    class="card-root relative shadow-xl w-60 overflow-hidden flex flex-col justify-center items-center hover:cursor-pointer hover:scale-[103%] transition-transform px-3 py-3 border bg-white"
  >
    <!--Add cardlist wishlist-->
    <div class="w-full h-14 absolute top-0 -translate-y-full transition-transform delay-200 flex justify-between items-center px-3 add-card">
      <AddCartButton :item="product" />
      <AddWishlistButton :item="product" />
    </div>
    <NuxtLink :to="`/products/${product.id}`">
      <img :src="product.images" :alt="product.category" class="w-full h-56 object-contain" />
